...
- 新增 `commons_redis_lock_manager` crate，提供基于 `commons_redis::Pool` 的
  Redis 分布式锁能力，并由根 crate `commons` 统一重导出。

### 修复

- 增强 `commons_redis` 的 Sentinel 恢复能力，补齐 Sentinel 节点自身连接配置， 支
  持将 Sentinel 节点与 Redis 数据节点的认证和连接参数分开设置。
